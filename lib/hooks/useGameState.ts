...
import type { ActiveGameData, Match, Player, Score } from "../types";
...
export interface GameState {
  ...
  setIsFillFromAttendeesLoading: (loading: boolean) => void;
  setIsRandomizeTeamsLoading: (loading: boolean) => void;
...
export function useGameState(): GameState {
  ...
  const [userRequestedMatchesList, setUserRequestedMatchesList] = useState(
    false,
  );
  ...
  const [isFillFromAttendeesLoading, setIsFillFromAttendeesLoading] = useState(
    false,
  );
  const [isRandomizeTeamsLoading, setIsRandomizeTeamsLoading] = useState(false);
  ...
  const stableMatch = useMemo(() => {
    if (
      currentGameContext?.type !== "active" ||
    ...
    currentGameContext?.type,
    currentGameContext?.gameData?.match?.id,
    currentGameContext?.gameData?.match?.match_status,
    currentGameContext?.gameData?.match?.start_time,
  ]);
  ...
  useEffect(() => {
    ...
    if (
      prevActiveGameId.current === activeGameId &&
      prevActiveGameStatus.current === activeGameStatus
    ) {
      return;
    }
    ...
    if (activeGame && currentGameContext?.matchId === activeGame.match.id) {
      setCurrentGameContext({
        type:
          activeGame.match.match_status === "active" ||
            activeGame.match.match_status === "paused"
            ? "active"
            : "planned",
        gameData: activeGame,
    ...
    if (
      isStartingGame && startingGameId &&
      activeGame?.match.id === startingGameId
    ) {
    ...
    if (!loading && !hasInitialized) {
      ...
      if (urlGameId) {
        ...
        loadGameById(urlGameId).then((gameData) => {
          if (gameData) {
            const contextType =
              (gameData.match.match_status === "active" ||
                  gameData.match.match_status === "paused")
                ? "active"
                : "planned";
            setCurrentGameContext({
              type: contextType,
  ...
  // Computed values
  const currentGameData = (currentGameContext?.gameData ?? null) as
    | ActiveGameData
    | null;
  ...
  const teamAScore = currentGameData
    ? currentGameData.scores.filter((score: Score) => score.team === "A").length
    : 0;
  const teamBScore = currentGameData
    ? currentGameData.scores.filter((score: Score) => score.team === "B").length
    : 0;
  ...
  return {
    ...
    setIsEndingGame,
    setIsLoadingFromURL,
    setIsFillFromAttendeesLoading,
    setIsRandomizeTeamsLoading,
