...
export function useDragAndDrop({ onSwitchPlayerTeam }: UseDragAndDropProps) {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging && dragState) {
        ...
        const isOverValidZone =
          !!(teamContainer || playerTile || dropPlaceholder || goalkeeperTile);
        ...
        if (isOverValidZone && goalkeeperTile) {
          ...
          if (teamAttribute) {
            ...
            const playerTiles = teamContainer.querySelectorAll(
              "[data-player-tile]",
            );
            ...
            const visiblePlayerTiles = Array.from(playerTiles).filter(
              (tile) => {
                ...
                if (isCrossTeamMove) {
                  ...
                  return tileElement.style.display !== "none" &&
                    !tileElement.hasAttribute("data-being-dragged");
                }
              },
            );
            const currentPlayers = visiblePlayerTiles.map((_, index) => ({
              id: `temp-${index}`,
            })) as Player[];

            const dropIndex = calculateDropPosition(
              syntheticEvent,
              teamAttribute as "A" | "B",
              currentPlayers,
              isCrossTeamMove,
            );
      ...
      if (isDragging) {
        ...
        setTimeout(() => {
          if (isDragging) { // Check if still dragging (might have been cleaned up by handleDragEnd)
            ...
            if (
              dragState && dragState.currentTeam !== null &&
              dragState.currentIndex !== null
            ) {
              onSwitchPlayerTeam(
                dragState.player,
                dragState.currentTeam,
  ...
  const calculateDropPosition = (
    ...
    const goalkeeperTile = teamContainer.querySelector(
    ...
    const allPlayerTiles = Array.from(
      teamContainer.querySelectorAll("[data-player-tile]"),
    ).filter((tile) => {
      const tileElement = tile as HTMLElement;
      return tileElement.style.display !== "none";
    });
    ...
    if (!isCrossTeamMove && dragState) {
      ...
      for (
        let filteredIndex = 0;
        filteredIndex < playerTiles.length;
        filteredIndex++
      ) {
    ...
    if (isCrossTeamMove) {
      ...
      if (
        dragState && dragState.originalIndex !== -1 &&
        dropIndex > dragState.originalIndex
      ) {
        dropIndex += 1;
      }
  ...
  const handleDragOver = (
    ...
    if (dragState) {
      ...
      if (goalkeeperTile) {
        ...
        dropIndex = calculateDropPosition(
          e,
          team,
          currentPlayers,
          isCrossTeamMove,
  ...
  const handleDragLeave = (e: React.DragEvent) => {
    ...
    if (
      mouseX < rect.left || mouseX > rect.right || mouseY < rect.top ||
      mouseY > rect.bottom
    ) {
      setDragState((prev) =>
        prev
